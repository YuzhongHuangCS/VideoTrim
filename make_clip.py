...
def find_peaks(start, end, cuts, vis_file, pos=True):
    start_peak = []
    end_peak = []
    ...
    for i in range(0, len(start_peak)):
        ...
        frame_start_next = None

        if (i+1) != len(start_peak):
            frame_start_next = start_peak[i+1]
        else:
            frame_start_next = sys.maxsize

        for j in range(0, len(end_peak)):
            frame_end = end_peak[j]

            if frame_end > frame_start:
                #if pos and frame_end >= frame_start_next:
                #    break
...
for test_video_file in test_filenames:
    ...
    vis_file = 'vimeo_raw2/' + test_name + '.mp4'
    ...
    find_peaks(start, end, cuts_pos, vis_file, pos=True)

    start_flip = 1 - np.asarray(start)
    end_flip = 1 - np.asarray(end)
    find_peaks(start_flip, end_flip, cuts_neg, vis_file, pos=False)
...
filename_list = list(cuts_pos.keys() & cuts_neg.keys())
...
for name, ary in config.items():
    for i, vis_file in enumerate(filename_list):
        ...
        cmd = f'ffmpeg -ss {time_start} -t {duration} -i {vis_file} -vcodec libx264 -an clips/{i}_{name_map[name]}.mp4'
        os.system(cmd)
