...
def main(args):
    ...
    pos_json = json.loads(open('map_data/pos_map_yt_start.json').read())
    for key, value in pos_json.items():
        [source, name, end_time] = value
        pos_dict[name].append(key)
    
    neg_dict = collections.defaultdict(list)
    neg_json = json.loads(open('map_data/neg_map_yt_start.json').read())
    ...
    with open('names_test.json', 'w') as fout:
        json.dump(names_train_list, fout)
 
    pos_valid = format_video(flatten_nested_list([v for k, v in pos_dict.items() if k in names_valid]), 'cache_yt_start/pos_')
    neg_valid = format_video(flatten_nested_list([v for k, v in neg_dict.items() if k in names_valid]), 'cache_yt_start/neg_')
    pos_test = format_video(flatten_nested_list([v for k, v in pos_dict.items() if k in names_test]), 'cache_yt_start/pos_')
    neg_test = format_video(flatten_nested_list([v for k, v in neg_dict.items() if k in names_test]), 'cache_yt_start/neg_')

    pos_train = format_video(flatten_nested_list([v for k, v in pos_dict.items() if k not in names_valid_test]), 'cache_yt_start/pos_')
    neg_train = format_video(flatten_nested_list([v for k, v in neg_dict.items() if k not in names_valid_test]), 'cache_yt_start/neg_')
    ...
    for epoch in range(n_epoch):
        ...
        if epoch % args.test_interval == 0 or epoch == (n_epoch-1):
            ...
            torch.save(model.state_dict(), f'model/start_resnet_18_xentropy_epoch_{epoch}.pth')
...
if __name__ == '__main__':
    ...
    parser.add_argument('-n_valid', default=64, type=int, help='n_valid')
