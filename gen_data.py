...
def generate_visualization(source, path):
    ...
    for name in ids[:20]:
        ...
        os.makedirs(f'vis_begin/{source}/{name}', exist_ok=True)
        start_times = config['start_times']
        for idx, start_time in enumerate(start_times):
            cmd = f'ffmpeg -y -ss {start_time} -t 1.91 -i {video_path} -vf "fps=fps=10" -f gif -s 720x480 vis_begin/{source}/{name}/{idx}.gif'
            print(cmd)
            os.system(cmd)
        
        n_cuts = len(start_times)
        html_filename = f'vis_begin/{source}/{name}/index.html'
