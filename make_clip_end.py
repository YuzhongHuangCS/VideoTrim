...
def find_peaks(start, end, cuts, vis_file):
    ...
    for i in range(0, len(end_peak)):
        frame_end = end_peak[i]
        score = end[frame_end]
        time_end = frame_end / 8
        cuts[vis_file].append([time_end-2, time_end, score])
...
for name, ary in config.items():
    for i, vis_file in enumerate(filename_list):
        ...
        cmd = f'ffmpeg -ss {time_start} -t {duration} -i {vis_file} -vcodec libx264 -an clips_end/{i}_{name_map[name]}.mp4'
        cmds.append(cmd)
        os.system(cmd)
