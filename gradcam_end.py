...
if __name__ == '__main__':
	...
	for f in fname:
		...
		for i_end in range(16, n):
			...
			os.makedirs(f'cam3_end_true/{f}', exist_ok=True)
			ret1 = []
			ret2 = []
			for j in range(16):
				img = X[0, :, j, :, :]
				...
				show_cam_on_image(img, mask[j], ret1, ret2, fname=f'cam3_end_true/cam_{i_end}/{j}.jpg')
			montages  = imutils.build_montages(ret1 + ret2, (112, 112), (16, 2))
			cv2.imwrite(f'cam3_end_true/{f}/cam_{i_end}.jpg', montages[0])
			#pdb.set_trace()
			print(i_end)
	'''
