...
def main(args):
    ...
    names_test = set(['youtube_c032f248-2b5e-4019-9c5f-672f3769dd82'])
    ...
    valid_loader = DataLoader(valid_dataset, batch_size = batch_size, num_workers=6, pin_memory=True, shuffle=False)
    test_loader = DataLoader(test_dataset, batch_size = batch_size, num_workers=6, pin_memory=True, shuffle=False)
    ...
    for epoch in range(n_epoch):
        ...
        if epoch % args.test_interval == 0 or epoch == (n_epoch-1):
            ...
            with open('end_prob_main_debug.json', 'w') as fout:
                json.dump(Y_end_prob_ary, fout)
            exit()
            #torch.save(model.state_dict(), f'model/exa_resnet_18_xentropy_epoch_{epoch}.pth')
