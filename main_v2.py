...
def main(args):
    ...
    pos_json = json.loads(open('map_data/.json').read())
    ...
    valid_loader = DataLoader(valid_dataset, batch_size = batch_size, num_workers=6, pin_memory=True, shuffle=True)
    test_loader = DataLoader(test_dataset, batch_size = batch_size, num_workers=6, pin_memory=True, shuffle=True)
    ...
    for epoch in range(n_epoch):
        ...
        for i_batch, sample in enumerate(train_loader):
            ...
            if args.loss_method == 'joint' or epoch < args.n_epoch_pretrain:
                emb = emb_raw / torch.sqrt(torch.sum(emb_raw**2, dim=1)).reshape([-1, 1])
                ...
                if use_contrastive:
                    ...
                    if n_group > 0:
                        ...
                        exp = torch.exp(distance_matrix / args.temperature)
