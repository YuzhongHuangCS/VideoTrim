...
pos_valid = format_video(flatten_nested_list([v for k, v in pos_dict.items() if k in names_valid]), 'cache/pos_')
neg_valid = format_video(flatten_nested_list([v for k, v in neg_dict.items() if k in names_valid]), 'cache/neg_')
pos_test = format_video(flatten_nested_list([v for k, v in pos_dict.items() if k in names_test]), 'cache/pos_')
neg_test = format_video(flatten_nested_list([v for k, v in neg_dict.items() if k in names_test]), 'cache/neg_')

pos_train = format_video(flatten_nested_list([v for k, v in pos_dict.items() if k not in names_valid_test]), 'cache/pos_')
neg_train = format_video(flatten_nested_list([v for k, v in neg_dict.items() if k not in names_valid_test]), 'cache/neg_')
...
for epoch in range(100):
    ...
    for i_batch, sample in enumerate(train_loader):
        ...
        label_ary.append(Y.cpu().detach().numpy())
    
    with open('train_emb.pickle', 'wb') as fout:
        pickle.dump([emb_ary, label_ary], fout)
    ...
    with open('valid_emb.pickle', 'wb') as fout:
        pickle.dump([emb_ary, label_ary], fout)
    ...
    with open('test_emb.pickle', 'wb') as fout:
        pickle.dump([emb_ary, label_ary], fout)
