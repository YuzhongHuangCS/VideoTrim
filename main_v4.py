...
class VideoSampler(Sampler):
    def __init__(self, data, pos_dict, neg_dict, pos_train, neg_train):
        #pdb.set_trace()
...
def main(args):
    ...
    sampler_train = VideoSampler(train_dataset, pos_dict, neg_dict, pos_train, neg_train)
    ...
    optimizer2 = optim.Adam(fcn.parameters(), lr=args.lr_finetune)
    ...
    for epoch in range(n_epoch):
        ...
        for i_batch, sample in enumerate(train_loader):
            ...
            optimizer.zero_grad()
            emb_raw = model(X)
            output = fcn(emb_raw.detach()[:len(Y)])
            loss = 0
...
if __name__ == '__main__':
    ...
    parser.add_argument('-n_epoch_pretrain', default=100, type=int, help='n_epoch_pretrain')
